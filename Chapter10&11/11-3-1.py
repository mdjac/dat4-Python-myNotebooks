...
inputs_ = [make_digit(raw_digit) for raw_digit in test_data]

print(inputs_)
print(len(inputs_))

for test_data in inputs_:
    result = predict(test_data)
    result = np.array(result)
    print(np.argmax(result), np.array_str(result, precision=2, suppress_small=True))

#DONE WITH PREDICTING MY DATA!!!!!! MDJAC
...
plot = plot_testset(inputs_)
plot.savefig('output.png', bbox_inches='tight')
#DONE WITH MY RESULT!!! MDJAC
